...
class GeminiEvaluator:
    ...
    def summarize_clusters(self, clusters_data: List[Dict[str, Any]], style: str = "toxic") -> str:
        ...
        请根据以下今日全网 AI 圈的海量聚类数据，识别出今日最具冲击力的 **所有大事件和核心矛盾**（不设上限，通常建议 8-15 个板块），并撰写一份极高密度的深度内参。

        原始数据背景:
        {summary_payload}

        要求：
        1. **极致密度，大事件驱动**：不要罗列小碎新闻。请将零散的聚类整合为一系列具有全局杀伤力的大板块。根据数据丰富度，动态拟定 8-15 个极具讽刺张力的标题，并使用 Markdown 的二级标题 (##) 格式。
        2. **深度穿透**：每一个板块下必须包含 3 个以上的深度分析段落。你要像手术刀一样切开每个热点的本质，揭露其背后的算力焦虑、融资困局或技术平庸。
        3. **极致毒舌**：直接、犀利、幽默。严禁流水账。
        4. **篇幅要求**：总字数必须确保在 3000 字左右，展现出绝对的深度、广度和对行业全方位的覆盖。
        5. **格式规范**：标题使用 ##，子标题使用 ###。严禁在标题文本中重复包含 ## 符号。
        6. **结尾**：以“## 【明日看戏指南：重点防忽悠指标】”给出 5 条监控指标。
        """

        # 🏛️ 正经战略版 Prompt
        official_prompt = f"""
        你是一个顶级 AI 行业智库的首席战略官，专门为跨国公司 CEO 和顶级投资者撰写战略内参。

        当前日期: {current_date}

        请根据以下今日全球 AI 圈的海量聚类数据，识别出今日最具战略影响力的 **全维度里程碑和范式位移**（不设上限，通常建议 8-15 个板块），并撰写一份极其详尽的战略综述。

        原始数据背景:
        {summary_payload}

        要求：
        1. **全维度全量扫描**：覆盖从算力、模型架构、行业落地到地缘竞争的所有核心变动。请自行拟定 **8-15 个宏大、专业的标题，并使用 Markdown 的二级标题 (##) 格式**。
        2. **深度逻辑整合**：每一个事件下必须包含 3-4 个深度段落，分析其对全球算力格局、底层架构演进 or 垂直产业重构的本质影响。展现极致的叙事张力。
        3. **智库口吻**：犀利、专业、具有战略高度，像高盛或麦肯锡内参。
        4. **篇幅要求**：总字数必须确保在 3000 字左右，确保决策者能获得足以支撑重大决策的详尽参考信息。
        5. **格式规范**：标题使用 ##，子标题使用 ###。严禁在标题文本中重复包含 ## 符号。
        6. **结尾**：以“## 【核心情报雷达：下阶段重点监控指标】”给出 5 条监控指标。
        """
