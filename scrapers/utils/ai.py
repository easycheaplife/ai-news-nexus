...
class GeminiEvaluator:
    ...
    def summarize_clusters(self, clusters_data: List[Dict[str, Any]], style: str = "toxic") -> str:
        ...
        请根据以下今日全网 AI 圈的海量聚类数据，识别出今日最具冲击力的 **6-10 个“大事件”或“核心矛盾”**，并撰写一份极高密度的深度内参。
        
        原始数据背景:
        {summary_payload}
        
        要求：
        1. **高密度大事件驱动**：不要罗列小碎新闻。请将零散的聚类整合为 **6-10 个具有全局杀伤力的大板块**。自行拟定极具讽刺张力的二级标题（##）。
        2. **深度穿透**：每一个板块下必须包含 3 个以上的深度分析段落。你要像手术刀一样切开每个热点的本质，指出谁在蹭热度、谁在掩盖亏损、谁的技术其实是过时货。
        3. **极致毒舌**：直接、犀利、幽默。严禁流水账，严禁官方口吻。
        4. **篇幅要求**：总字数必须确保在 2500 字以上，展现出绝对的深度和广度。
        5. **格式规范**：使用标准的 Markdown 二级标题 (##) 和三级标题 (###)。
        ...
        请根据以下今日全球 AI 圈的海量聚类数据，识别出今日最具战略影响力的 **6-10 个“核心里程碑”或“范式位移”**，并撰写一份全维度的战略综述。
        
        原始数据背景:
        {summary_payload}
        
        要求：
        1. **全维度战略驱动**：覆盖从算力、模型架构、应用落地到地缘竞争的方方面面。请自行拟定 **6-10 个宏大、专业的二级标题（##）**。
        2. **深度逻辑整合**：每一个事件下必须包含 3-4 个深度段落，分析其对全球算力格局、底层架构演进或垂直产业重构的本质影响。要有宏观的叙事张力。
        3. **智库口吻**：犀利、专业、富有前瞻性，字里行间体现出“情报价值”。
        4. **篇幅要求**：总字数必须确保在 2500 字以上，确保决策者能获得足够详尽的参考。
        5. **格式规范**：使用标准的 Markdown 二级标题 (##) 和三级标题 (###)。
